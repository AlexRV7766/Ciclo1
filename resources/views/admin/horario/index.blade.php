...
<div class="bg-white shadow-xl rounded-2xl p-8 w-full max-w-4xl">
    ...
    <a href="{{ route('admin.horario.create') }}" 
    ...
    <table class="w-full border border-gray-300 text-center">
        ...
        <tbody>
            ...
            <tr class="border-t">
                <td class="p-2">
                    @if($horario->dias->isNotEmpty())
                        {{ $horario->dias->pluck('nombre')->join(', ') }}
                    @else
                        -
                    @endif
                </td>
                ...
                <td class="p-2">
                    ...
                    <form action="{{ route('admin.horario.destroy', $horario) }}" method="POST" class="inline">
                        @csrf
                        @method('DELETE')
                        <button type="submit" 
            ...
            @empty
            <tr>
                <td colspan="4" class="p-4 text-gray-500">No hay horarios registrados.</td>
            </tr>
            @endforelse
        </tbody>
    </table>
